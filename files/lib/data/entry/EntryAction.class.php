...
class EntryAction extends AbstractDatabaseObjectAction
{
    ...
    public function create()
    {
        ...
        if (!empty($entry->location)) {
            $message .= ' ' . \str_replace(',', ' ', $entry->location);
        }
        if (!empty($optionSearch)) {
            $message .= ' ' . $optionSearch;
        }
        if (!empty($entry->text2)) {
            $message .= ' ' . $entry->text2;
        }
        if (!empty($entry->text3)) {
            $message .= ' ' . $entry->text2;
        }
        if (!empty($entry->text4)) {
            $message .= ' ' . $entry->text2;
        }
        if (!empty($entry->text5)) {
            $message .= ' ' . $entry->text2;
        }
        ...
        // trigger publication
        if (!$entry->isDisabled) {
            $action = new self([$entryEditor], 'triggerPublication');
    ...
    public function update()
    {
        ...
        foreach ($this->getObjects() as $entry) {
            ...
            // add log entry
            EntryModificationLogHandler::getInstance()->edit($entry->getDecoratedObject(), ($this->parameters['reason'] ?? ''));
            ...
            // use message for various other data (MEDIUMTEXT)
            $message = $this->parameters['data']['message'] ?? $message;
            $location = $this->parameters['data']['location'] ?? '';
            $text2 = $this->parameters['data']['text2'] ?? '';
            $text3 = $this->parameters['data']['text3'] ?? '';
            $text4 = $this->parameters['data']['text4'] ?? '';
            $text5 = $this->parameters['data']['text5'] ?? '';
            ...
            SearchIndexManager::getInstance()->set(
                ...
                $entry->entryID,
                $message,
                $this->parameters['data']['subject'] ?? $entry->subject,
                $entry->time,
                $entry->userID,
                $entry->username,
                $entry->languageID
    ...
    public function enable()
    {
        ...
        // publish entrys
        $entryAction = new self($this->objects, 'triggerPublication');
    ...
    public function validateGetMapMarkers()
    {
        ...
        } elseif (\is_string($this->parameters['excludedObjectIDs'])) {
            try {
        ...
        } elseif (!\is_array($this->parameters['excludedObjectIDs'])) {
            throw new UserInputException('excludedObjectIDs');
        }
